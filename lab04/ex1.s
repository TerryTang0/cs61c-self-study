...
main:
    ...
    li a0, 2    # a0 = 2
    li a1, 7    # a1 = 7
    jal ra pow    # Call pow function to get ra value
    li t0, 128 # verifies that pow returned the right value
    beq a0, t0, next_test     # If a0=t0, jump to next_test
    la a0, pow_string    # Load the address of power_string to a0
    j failure

next_test:
    # inc_arr: increments "array" in place
    la a0, array    # Load the address of array to a0
    li a1, 5    # a1 = 5
    jal ra inc_arr    
